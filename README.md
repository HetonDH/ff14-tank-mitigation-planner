...
FFLogs API 直连需要 OAuth 密钥和代理服务，当前版本先支持上传或粘贴 FFLogs 事件 JSON；本地 `.log/.txt` 为实验解析，需要用真实样本继续校对。
